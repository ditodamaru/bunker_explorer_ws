...
class TerminalLauncher(QtWidgets.QWidget):
    ...
    def setupUi(self):
        ...
        # Adding a line edit for custom arguments
        self.custom_arguments_edit = QtWidgets.QLineEdit()
        layout.addWidget(QtWidgets.QLabel("Custom Arguments:"))
        layout.addWidget(self.custom_arguments_edit)
    ...
    def startTerminal(self, index):
        commands = [
            ...
            "roscore",
            #"roslaunch /home/andi/catkin_test_ws/src/ublox_f9p/ublox_gps/launch/ublox_zed-f9p0.launch",
            "roslaunch ublox_gps ublox_zed-f9p0.launch",
            "rosbag record -a"
        ]
        ...
        if 0 <= index < len(commands):
            if index == 3:  # Terminal 3 (Map Saver)
                custom_arguments, ok = QtWidgets.QInputDialog.getText(self, "Save As the Map", "Enter map name:")
                if not ok:
                    return  # User canceled the input dialog, do not proceed with the process
                command = commands[index] + " " + custom_arguments
            else:
