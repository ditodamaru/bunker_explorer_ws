...
int main (int argc, char** argv)
{
    ...
    while(ros::ok())
    {
		...
		if (serial_fd >= 0) 
		{
			...
			imu_data_msg.header.frame_id = tf_prefix_ + "imu_link2";  // "imu_link"
			...
			if(m_bSingle_TF_option)
			{
				...
				br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), tf_prefix_ + "/robot_base", tf_prefix_ + "/imu_link"));
			}
