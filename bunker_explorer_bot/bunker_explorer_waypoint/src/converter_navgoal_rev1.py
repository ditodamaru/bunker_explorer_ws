...
if not waypoint_files:
    ...
    for i, filename in enumerate(waypoint_files):
        #print(f"{i + 1}. {filename}") #for python3
        print("{}. {}".format(i + 1, filename))

    # Ask the user to select a file for conversion
    while True:
        try:
            selection = int(input("Select a waypoint file (1-{len(waypoint_files)}): "))
    ...
    #output_file = os.path.join("$(find tracking_pid)/trajectories/", f"{os.path.splitext(selected_file)[0]}.yaml")
    #output_file = os.path.expandvars("$(find tracking_pid)/trajectories/") + os.path.splitext(selected_file)[0] + ".yaml"
    ...
    with open(output_file, 'w') as file:
        yaml.dump_all(waypoints, file, default_flow_style=False)

    #print(f"Waypoint data from {selected_file} has been converted and saved to {output_file}")
