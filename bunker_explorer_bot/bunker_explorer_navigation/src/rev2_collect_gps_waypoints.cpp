...
std::string end_button_num, collect_button_num;  // Changed to std::string

void joy_CB(const sensor_msgs::Joy joy_msg)
{
    if (joy_msg.buttons[0] == collect_button_num[0])  // Compare with first character of std::string
    {
    ...
    if (joy_msg.buttons[0] == end_button_num[0])  // Compare with first character of std::string
    {
...
{
    char key_pressed = msg->data;

    if (key_pressed == collect_button_num[0])  // Compare with first character of std::string
    {
        collect_request = true;
    }
    else if (key_pressed == end_button_num[0])  // Compare with first character of std::string
    {
        continue_collection = false;
    }
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_keyboard = n.subscribe("/keyboard_input", 1, keyboard_CB);
