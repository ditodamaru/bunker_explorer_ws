...
class TerminalLauncher(QtWidgets.QWidget):
    ...
    def setupUi(self):
        ...
        self.arguments = [
            ...
            "rosrun map_server map_saver map:=/rtabmap/proj_map -f /home/andi/catkin_devel_ws/src/bunker_explorer_bot/bunker_explorer_navigation/maps/",
            "",
            "",
            "",
            "database_path:=~/.ros/rtabmap2.db",
            "",
            "",
            "",
            "",
        ...
        for label, command, arguments in zip(self.labels, self.commands, self.arguments):
            button_layout = QtWidgets.QHBoxLayout()

            label_widget = QtWidgets.QLabel(label)
            button_layout.addWidget(label_widget)

            start_button = QtWidgets.QPushButton("Start Terminal")
            self.start_buttons.append(start_button)
            start_button.clicked.connect(lambda _, idx=len(self.start_buttons)-1: self.startTerminal(idx, command, arguments))
            button_layout.addWidget(start_button)

            if arguments:
                arguments_label = QtWidgets.QLabel(arguments)
                button_layout.addWidget(arguments_label)
    ...
    def startTerminal(self, index, command, arguments):
        if 0 <= index < len(self.commands):
            if index == 5:  # Terminal 3 (RTABMAP launch)
                ...
                command = self.commands[index] + " " + custom_arguments
                ...
                if not ok:
                    return  # User canceled the input dialog, do not proceed with the process
                commsand = self.commands[index] + " " + custom_arguments                
            else:
            ...
            if arguments:
                QtWidgets.QApplication.clipboard().setText(arguments)
