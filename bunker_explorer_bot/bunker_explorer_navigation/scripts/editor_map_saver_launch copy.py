    ...
    for node in root.findall(".//node[@name='map_saver_node']"):
        ...
        if args and args.startswith('-f $(find bunker_explorer_navigation)/maps/'):
            ...
            for i, arg in enumerate(args_list):
                if arg == '-f':
                    args_list[i + 1] = "$(find bunker_explorer_navigation)/maps/{}".format(new_map_filename)
                    node.set('args', ' '.join(args_list))
                    break
...
def main():
    try:
        ...
        if len(rtabmap_databases) == 0:
            ...
            # Prompt the user for the current map filename in the launch file
            current_map_filename = input("Enter the current map filename in the launch file: ")
